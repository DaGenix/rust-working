...
/// multiply a by b in GF(2^8) in constant time
fn mult(mut a: u8, mut b: u8) -> u8 {
    let mut p = 0u8;
    for 8.times() {
        let lowest_bit_is_zero = constant_time_eq(b & 1, 0);
        p ^= constant_time_select(lowest_bit_is_zero, 0, a);
        let carry = constant_time_eq(a & 0x80, 0x80);
        a <<= 1;
        a ^= constant_time_select(carry, 0x1b, 0);
        b >>= 1;
    }
    return p;
}

/// determine the multiplicitive inverse of x in GF(2^8) in constant time
fn mult_inv(x: u8) -> u8 {
    let mut out: u8 = 0;
    for uint::range(0, 256) |y| {
        let result = mult(x, y as u8);
        let is_mult_inverse = constant_time_eq(result, 1);
        out = constant_time_select(is_mult_inverse, y as u8, out);
    }
    return out;
}

/// rotate left by 1
fn ror_left_one(in: u8) -> u8 {
    return in << 1 | in >> 7;
}

/// Calculate the S_BOX value for in in constant time. Very slow!
pub fn calc_s(in: u32) -> u32 {
    let mut s = mult_inv(in as u8);
    let mut x = s;
    for 4.times() {
        s = s << 1 | s >> 7;
        x = s ^ x;
    }
    return (x ^ 0x63) as u32;
}

/// Calculate the S_INV_BOX value for in in constant time. Very, very slow!
pub fn calc_s_inv(in: u32) -> u32 {
    let mut out: u8 = 0;
    for uint::range(0, 256) |x| {
        let is_inv = constant_time_eq(calc_s(x as u32) as u8, in as u8);
        out = constant_time_select(is_inv, x as u8, out);
    }
    return out as u32;
}
...
mod test {
    ...
    #[test]
    fn test_gen_s_box() {
        let mut gen = [0u32, ..256];
        for u32::range(0, 256) |x| {
            gen[x] = calc_s(x);
        }
        assert!(S == gen);
    }
